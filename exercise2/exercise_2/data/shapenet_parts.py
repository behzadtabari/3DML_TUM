...
class ShapeNetParts(torch.utils.data.Dataset):
    ...
    def get_point_cloud_with_labels(shapenet_id):
        ...
        # get point cloud
        point_path = str(ShapeNetParts.dataset_path) + "/" + category_id + "/points/" + shape_id + ".pts"
        ...
        label_list = ShapeNetParts.part_id_to_overall_id
        label_path = str(ShapeNetParts.dataset_path) + "/" + category_id + "/points_label/" + shape_id + ".seg"
        labels = np.loadtxt(label_path).astype(np.int).T
        labels = labels[random_indices]
        return point_cloud, labels


# TODO: Important: Use ShapeNetParts.part_id_to_overall_id to convert the part labels you get from the .seg files
# from local to global ID as they start at 0 for each shape class whereas we want to predict the overall part class.
# ShapeNetParts.part_id_to_overall_id converts an ID in form <shapenetclass_partlabel> to and integer representing
# the global part class id
