...
def train(model, trainloader, valloader, device, config):
    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            # Add missing pieces, as in the exercise parts before
            ...
            loss = loss_criterion(prediction.view(-1, 50), batch['segmentation_labels'].view(-1))
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    correct += (predicted_labels == batch_val['segmentation_labels']).sum().item()

                    part_ious = []
                    for part in range(ShapeNetParts.num_classes):
                        I = torch.sum(torch.logical_and(predicted_labels == part, batch_val['segmentation_labels'] == part))
                        U = torch.sum(torch.logical_or(predicted_labels == part, batch_val['segmentation_labels'] == part))
                        if U == 0:
