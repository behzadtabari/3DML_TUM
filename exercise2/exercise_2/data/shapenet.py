...
class ShapeNetPoints(torch.utils.data.Dataset):
    ...
    def __init__(self, split = 'train'):
        # Read sample IDs from the correct split file and store in self.items
        super().__init__()
        assert split in ['train', 'val', 'overfit']
    ...
    def get_point_cloud(shapenet_id):
        ...
        tr_point_cloud = trimesh.load(path)
        point_cloud = np.array(tr_point_cloud.vertices).astype(np.float32)
